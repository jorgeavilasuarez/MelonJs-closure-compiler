...
goog.provide('me.loader');
goog.require('me.utils');
 ...
 */
me.loader.checkLoadStatus = function() {
    if (loadCount === resourceCount) {
        ...
        timerId = setTimeout(me.loader.checkLoadStatus, 100);
    }
};
 ...
 * @returns {undefined}
 */
me.loader.preloadImage = function(img, onload, onerror) {
    // create new Image me.loaderect and add to list
    imgList[img.name] = new Image();
    imgList[img.name].onload = onload;
    imgList[img.name].onerror = onerror;
    imgList[img.name].src = img.src + me.loader.nocache;
};

/**
 * preload TMX files 
 * @param {Object} tmxData
 * @param {Object} onload
 * @param {Object} onerror
 * @returns {undefined}
 */
me.loader.preloadTMX = function(tmxData, onload, onerror) {
    ...
    xmlhttp.open("GET", tmxData.src + me.loader.nocache, true);
 ...
 * @returns {undefined}
 */
me.loader.preloadJSON = function(data, onload, onerror) {
    var xmlhttp = new XMLHttpRequest();
    ...
    xmlhttp.open("GET", data.src + me.loader.nocache, true);
 ...
 * @returns {undefined}
 */
me.loader.preloadBinary = function(data, onload, onerror) {
    var httpReq = new XMLHttpRequest();

    // load our file
    httpReq.open("GET", data.src + me.loader.nocache, true);
 ...
 */
me.loader.onProgress = undefined;
 ...
 * @returns {undefined}
 */
me.loader.onResourceLoaded = function(e) {

    // increment the loading counter
    loadCount++;

    // callback ?
    var progress = me.loader.getLoadProgress();
    if (me.loader.onProgress) {
        // pass the load progress in percent, as parameter
        me.loader.onProgress(progress);
    }
 ...
 * @returns {undefined}
 */
me.loader.onLoadingError = function(res) {
 ...
 * @returns {undefined}
 */
me.loader.setNocache = function(enable) {
    me.loader.nocache = enable ? "?" + parseInt(Math.random() * 10000000, 10) : '';
};
 ...
 * @public
 * @function
 * @param {Array.<string>} res resources
 * @example
 * var g_resources = [ 
 *   // PNG tileset
 ...
 */
me.loader.preload = function(res) {
    // parse the resources
    for (var i = 0; i < res.length; i++) {
        resourceCount += me.loader.load(res[i], me.loader.onResourceLoaded.bind(me.loader), me.loader.onLoadingError.bind(me.loader, res[i]));
    }
    // check load status
    me.loader.checkLoadStatus();
};
 ...
 * @param {Function} onload onload function to be called when the resource is loaded
 ...
 * });
 */
me.loader.load = function(res, onload, onerror) {
    ...
    switch (res.type) {
        case "binary":
            ...
            me.loader.preloadBinary(res, onload, onerror);
            return 1;

        case "image":
            // reuse the preloadImage fn
            me.loader.preloadImage(res, onload, onerror);
            return 1;

        case "json":
            me.loader.preloadJSON(res, onload, onerror);
            return 1;

        case "tmx":
        case "tsx":
            me.loader.preloadTMX(res, onload, onerror);
            return 1;
 ...
 */
me.loader.unloadAll = function() {
    var name;

    // unload all binary resources
    for (name in binList)
        me.loader.unload(name);
...
/**
 * return the specified TMX me.loaderect storing type
 * @name getTMXFormat
 * @memberOf me.loader
 * @public
 * @function
 * @param {string} elt tmx name of the tmx/tsx element ("map1");
 * @return {string} 'xml' or 'json'
 */
me.loader.getTMXFormat = function(elt) {
    // avoid case issue
    elt = elt.toLowerCase();
    if (elt in tmxList)
...
/**
 * return the specified TMX/TSX me.loaderect
 * @name getTMX
 * @memberOf me.loader
 * @public
 * @function
...
/**
 * return the specified Binary me.loaderect
 * @name getBinary
 * @memberOf me.loader
 * @public
 * @function
 * @param {string} elt name of the binary me.loaderect ("ymTrack");
 * @return {Object} 
 */
me.loader.getBinary = function(elt) {
    // avoid case issue
    elt = elt.toLowerCase();
    if (elt in binList)
        return binList[elt];
 ...
 * @public
 * @function
 * @param {string} elt Image name of the Image element ("tileset-platformer");
 * @return {Image} 
 */
me.loader.getImage = function(elt) {
    // avoid case issue
    elt = elt.toLowerCase();
    if (elt in imgList) {
        // return the corresponding Image me.loaderect
        return imgList[elt];
    } else {
        //console.log ("warning %s resource not yet loaded!",name);
        return null;
 ...
 * @public
 * @function
 * @param {string} elt Name for the json file to load
 * @return {Object} 
 */
me.loader.getJSON = function(elt) {
    elt = elt.toLowerCase();
    if (elt in jsonList) {
        return jsonList[elt];
    }
 ...
 * @return {number} 
 */
me.loader.getLoadProgress = function() {
