...
me.device._check = function() {
    ...
    me.device.hasAccelerometer = (
            ...
            (typeof (window.Windows.Devices.Sensors.Accelerometer) === 'function')
            )
            );
    ...
    if (me.device.hasAccelerometer) {
        if (!accelInitialized) {
            if (typeof window.Windows === 'undefined') {
                ...
                // On Windows 8 Device
                var accelerometer = window.Windows.Devices.Sensors.Accelerometer.getDefault();
                if (accelerometer) {
...
me.device.unwatchAccelerometer = function() {
    if (accelInitialized) {
        if (typeof window.Windows === 'undefined') {
            ...
            var accelerometer = window.Windows.Device.Sensors.Accelerometer.getDefault();
