...
// (a polyfill will probably be required at some stage, once this will throttlebe fully standardized
...
function enablePointerEvent() {
    if (!pointerInitialized) {
        ...
        me.input.changedTouches.push({x: 0, y: 0});
        me.input.mouse.pos = new me.Vector2d(0, 0);
        ...
        if (me.input.throttlingInterval === undefined) {
            // set the default value
            me.input.throttlingInterval = Math.floor(1000 / me.sys.fps);
        }
        // if time interval <= 16, disable the feature
        if (me.input.throttlingInterval < 17) {
            me.video.getScreenCanvas().addEventListener(activeEventList[1], onMoveEvent, false);
        }
        else {
            me.video.getScreenCanvas().addEventListener(activeEventList[1], throttle(me.input.throttlingInterval, false, function(e) {
                onMoveEvent(e);
            }), false);
        }
...
function dispatchEvent(e) {
    ...
    if (handlers) {
        ...
        for (var t = 0, l = me.input.changedTouches.length; t < l; t++) {
            ...
            if (!navigator.pointerEnabled) {
                ...
                e.pointerId = me.input.changedTouches[t].id;
            }

            /* Initialize the two coordinate space properties. */
            e.gameScreenX = me.input.changedTouches[t].x;
            e.gameScreenY = me.input.changedTouches[t].y;
...
function updateCoordFromEvent(e) {
    ...
    me.input.changedTouches.length = 0;

    // PointerEvent or standard Mouse event
    if (!e.touches) {
        local = me.input.globalToLocal(e.clientX, e.clientY);
        local.id = e.pointerId || 1;
        me.input.changedTouches.push(local);
    }
    // iOS/Android like touch event
    else {
        for (var i = 0, l = e.changedTouches.length; i < l; i++) {
            var t = e.changedTouches[i];
            local = me.input.globalToLocal(t.clientX, t.clientY);
    ...
    me.input.mouse.pos.set(me.input.changedTouches[0].x, me.input.changedTouches[0].y);
}
...
function onPointerEvent(e) {
    ...
    var keycode = me.input.mouse.bind[e.button || 0];
 ...
 * @memberOf me.input
 */
me.input.throttlingInterval = undefined;
 ...
 *
 */
me.input.isKeyPressed = function(action) {
    if (keyStatus[action]) {
        if (keyLock[action]) {
 ...
 */
me.input.keyStatus = function(action) {
 ...
 */
me.input.triggerKeyEvent = function(keycode, status) {
    if (status) {
        keydown({}, keycode);
    }
    else {
 ...
 */
me.input.bindKey = function(keycode, action, lock) {
 ...
 * }
 */
me.input.unlockKey = function(action) {
    keyLocked[action] = false;
};
 ...
 */
me.input.unbindKey = function(keycode) {
 ...
 * };
 */
me.input.globalToLocal = function(x, y) {
 ...
 */
me.input.bindMouse = function(button, keyCode) {
 ...
 */
me.input.unbindMouse = function(button) {
    // clear the event status
 ...
 */
me.input.bindTouch = function(keyCode) {
    // reuse the mouse emulation stuff
    // where left mouse button is map to touch event
    me.input.bindMouse(me.input.mouse.LEFT, keyCode);
};
 ...
 */
me.input.registerPointerEvent = function(eventType, rect, callback, floating) {
 ...
 * @param {me.Rect} rect region object target (or corresponding region defined through me.Rect)
 * @example
 * // release the registered object/region on the 'mousemove' event
 * me.input.releasePointerEvent('mousemove', this.collisionBox);
 */
me.input.releasePointerEvent = function(eventType, rect) {
